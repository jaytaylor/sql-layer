...
run_physical_plan("Dump the entire group",
         GroupScan(coi))

run_physical_plan("Scan the group, selecting customers named tom",
         Select(GroupScan(coi), Tc, lambda customer: customer.name == 'tom'))

run_physical_plan("Scan the group, selecting orders made in January",
         Select(GroupScan(coi), To, lambda order: order.order_date.startswith('2010/1')))

run_physical_plan("Scan the customer name index",
...
run_physical_plan("Flatten customer and order",
         Flatten(GroupScan(coi), Tc, To, Tco))

run_physical_plan("Flatten customer and order, dropping customers with no orders",
         Flatten(GroupScan(coi), Tc, To, Tco, INNER_JOIN))

run_physical_plan("Flatten customer and order, keeping orders with no customers",
         Flatten(GroupScan(coi), Tc, To, Tco, RIGHT_JOIN))

run_physical_plan("Flatten customer and order and drop items",
         Cut(Flatten(GroupScan(coi), Tc, To, Tco), Ti))

run_physical_plan("Find the customer named 'jack' and flatten",
         Flatten(Select(GroupScan(coi), Tc, lambda customer: customer.name == 'jack'), Tc, To, Tco))

run_physical_plan("Find the customer named 'jack' and flatten, dropping jack if he has no orders (he doesn't)",
...
run_physical_plan("Select after Flatten",
         Select(
             Flatten(GroupScan(coi), Tc, To, Tco),
             Tco,
...
run_physical_plan("select c.name, o.order_date from customer c, order o where o.cid = c.cid and o.order_date like '2010/1/%'",
         Project
         (Flatten
          (Project
           (Project

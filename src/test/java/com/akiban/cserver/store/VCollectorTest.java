        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                if (rowDef.getRowDefId() == 1003) {
                    GroupGenerator dbGen = new GroupGenerator(VCOLLECTOR_TEST_DATADIR, ais, rowDefCache);
                    dbGen.generateGroup(rowDef);
                    ArrayList<RowData> rowData = dbGen.getRows();
                    
                    VCollector vc = new VCollector(dbGen.getMeta(), rowDefCache, rowDef.getRowDefId(), dbGen.getGroupBitMap());
                    ...
                    while (j.hasNext()) {
                        ...
                        while (k < actual.length) {
                            System.out.print(Integer.toHexString(actual[k])+" "); k++;
                        }
                        System.out.println();
                          */  

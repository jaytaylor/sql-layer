...
public class CutIT extends PhysicalOperatorITBase
{
    @Before
    public void before()
    {
    ...
    public void testNullCutType()
    {
        cut_Default(groupScan_Default(coi), null);
    }
    ...
    {
        PhysicalOperator plan = cut_Default(groupScan_Default(coi), customerRowType);
    ...
    {
        PhysicalOperator plan = cut_Default(groupScan_Default(coi), orderRowType);
        Cursor cursor = cursor(plan, adapter);
        RowBase[] expected = new RowBase[]{
            row(customerRowType, 1L, "northbridge"),
    ...
    {
        PhysicalOperator plan = cut_Default(groupScan_Default(coi), itemRowType);
        Cursor cursor = cursor(plan, adapter);
        RowBase[] expected = new RowBase[]{
            row(customerRowType, 1L, "northbridge"),
            row(orderRowType, 11L, 1L, "ori"),
    ...
    {
        PhysicalOperator plan = cut_Default(groupScan_Default(coi), addressRowType);
        Cursor cursor = cursor(plan, adapter);
        RowBase[] expected = new RowBase[]{
            row(customerRowType, 1L, "northbridge"),
            row(orderRowType, 11L, 1L, "ori"),

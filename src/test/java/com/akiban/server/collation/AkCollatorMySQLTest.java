...
public class AkCollatorMySQLTest {
    ...
    public void keyEncoding() throws Exception {
        final AkCollator collator = AkCollatorFactory.getAkCollator("latin1_swedish_ci");
        final Key key1 = new Key((Persistit) null);
    ...
    private void verifySequence(final String collation, final Integer[] expected) {
        ...
        for (int i = 0; i < 256; i++) {
            collator.append(key1.clear(), new String(new char[] { (char) i }));
            key1.append(i);
            map.put(new KeyState(key1), i);
        }
    ...
    private void testRandomStringSequence(final String collation, final int count, final Integer[] table) throws Exception {
        ...
        assertArrayEquals("Comparators should match", map1.values().toArray(), map2.values().toArray());
    }

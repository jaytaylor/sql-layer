...
public class OnlineCreateTableAsCastedMT extends OnlineCreateTableAsBase {
    ...
    private int ctID;
    private String CAST_TABLE = "ct";

    @Before
    public void createAndLoad() {
        CREATE_QUERY = " CREATE TABLE " + TO_TABLE + " AS SELECT CAST( ABS(id) AS DOUBLE ) , x IS NOT NULL FROM " + FROM_TABLE + " WITH DATA ";
        ftID = createTable(SCHEMA, FROM_TABLE, "id INT NOT NULL PRIMARY KEY, x INT");
        ctID = createTable(SCHEMA, CAST_TABLE, "id INT NOT NULL PRIMARY KEY, x BOOLEAN");

        fromTableRowType = SchemaCache.globalSchema(ais()).tableRowType(ftID);
        toTableRowType = SchemaCache.globalSchema(ais()).tableRowType(ctID);

        writeRows(createNewRow(ftID, -1, 10),
                createNewRow(ftID, 2, 20),
                createNewRow(ftID, 3, 30),
                createNewRow(ftID, 4, 40),
                createNewRow(ctID, 1, true),
                createNewRow(ctID, 2, true),
                createNewRow(ctID, 3, true),
                createNewRow(ctID, 4, true));

        fromGroupRows = runPlanTxn(groupScanCreator(ftID));//runs given plan and returns output row
        toGroupRows = runPlanTxn(groupScanCreator(ctID));
    ...
    protected OperatorCreator getOtherCreator() {
        ttID = ais().getTable(SCHEMA, CAST_TABLE).getTableId();
    ...
    public void castedInsertPreToPostMetadata() {
        ...
        otherGroupRows = getOtherExpected();
    ...
    public void castedInsertPostMetaToPreFinal() {
        ...
        otherGroupRows = combine(getOtherExpected(), newCastRow);

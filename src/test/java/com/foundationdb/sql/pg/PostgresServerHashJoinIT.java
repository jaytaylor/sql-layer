...
public class PostgresServerHashJoinIT extends PostgresServerITBase{

    private static final String SQL = "SELECT items.sku, categories.cat FROM items JOIN categories on items.sku = categories.sku AND categories.cat = 1 ORDER BY items.sku";
    ...
    public void test() throws Exception {
        ...
        boolean foundUsing = false;
        boolean foundLookup = false;
        while ((!foundUsing || !foundLookup) && planRS.next()) {
            if (((String) planRS.getObject(1)).contains("Using_HashTable"))
                foundUsing = true;
            if (((String) planRS.getObject(1)).contains("HashTableLookup_Default"))
                foundLookup = true;
        }
        assert(foundUsing);

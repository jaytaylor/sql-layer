...
public abstract class Task
{
    ...
    // parentColumns are columns in join.parent. Return the corresponding columns in join.child. If a column is
    ...
    // Returns table columns that participate in the hkey. Result might not be a complete hkey.
    ...
    protected static String commaSeparatedColumnDeclarations(List<Column> columns)
    {
        StringBuilder buffer = new StringBuilder();
        boolean first = true;
        for (Column column : columns) {
    ...
    private static List<Column> hKeyColumns(Join join)
    {
        ...
        List<Column> parentHKeyColumns =
            joinParent.getParentJoin() == null
            ? joinParent.getPrimaryKey().getColumns()
            : hKeyColumns(joinParent.getParentJoin());
        // if hkey column in parent has no counterpart in child, it is dropped by columnsInChild.

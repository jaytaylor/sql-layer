...
public class BulkLoader extends Thread {
    ...
    public void run() {
        try {
            DB db = dbHost == null ? null : new DB(dbHost, dbPort, dbUser,
                    dbPassword);
            prepareWorkArea(db);
            tracker = new Tracker(db, artifactsSchema);
            tracker
                    .info(
                            "Starting bulk load, source: %s@%s:%s, groups: %s, resume: %s, cleanup: %s",
            ...
            IdentityHashMap<UserTable, TableTasks> tableTasksMap = new TaskGenerator(
                    this, taskGeneratorActions).generateTasks();
            ...
            new PersistitLoader(persistitStore, db, tracker)
                    .load(finalTasks(tableTasksMap));
    ...
    public static synchronized BulkLoader start(Store store,
            AkibaInformationSchema ais, List<String> groups,
            String artifactsSchema, Map<String, String> sourceSchemas,
            String dbHost, int dbPort, String dbUser, String dbPassword,
            boolean resume, boolean cleanup) throws ClassNotFoundException,
            SQLException, InProgressException {
        if (inProgress == null) {

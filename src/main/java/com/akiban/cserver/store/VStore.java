 ...
 * 
 */
public class VStore implements Store {
	 ...
	 */
	@Override
	public List<RowData> fetchRows(String schemaName, String tableName,
			String columnName, Object least, Object greatest,
			String leafTableName) throws Exception {
		return hstore.fetchRows(schemaName, tableName, columnName, least,
				greatest, leafTableName);
	}
	 ...
	 */
	@Override
	public long getRowCount(boolean exact, RowData start, RowData end,
			byte[] columnBitMap) throws Exception {
	 ...
	 */
	@Override
	public RowCollector newRowCollector(int rowDefId, int indexId,
			int scanFlags, RowData start, RowData end, byte[] columnBitMap)
			throws Exception {
		return hstore.newRowCollector(rowDefId, indexId, scanFlags, start, end,
				columnBitMap);
	}
	 ...
	 */
	@Override
	public int writeRowForBulkLoad(Exchange hEx, RowDef rowDef,
			RowData rowData, int[] ordinals, FieldDef[][] fieldDefs,
			Object[][] hKey) throws Exception {
		return hstore.writeRowForBulkLoad(hEx, rowDef, rowData, ordinals,
				fieldDefs, hKey);
	}

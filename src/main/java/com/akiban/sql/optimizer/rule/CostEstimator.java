...
public abstract class CostEstimator implements TableRowCounts
{
                                      ...
                                      ExpressionNode highComparand, boolean highInclusive)
    {
        ...
        if (lowComparand != null || highComparand != null) {
            selectivity *= fractionBetween(index.getAllColumns().get(columnCount - 1),
    ...
    protected double fractionEqual(List<ExpressionNode> eqExpressions, 
        ...
        for (int column = 0; column < eqExpressions.size(); column++) {
            ...
            selectivity *= fractionEqual(index.getAllColumns().get(column),
    ...
    protected double fractionEqual(IndexColumn indexColumn,
                                   Index index, IndexStatistics indexStats,
                                   byte[] columnValue) {
        if (indexStats == null) {
            missingStats(indexColumn, index);
    ...
    protected double fractionBetween(IndexColumn indexColumn,
                                     Index index, IndexStatistics indexStats,
    ...
    protected void missingStats(IndexColumn indexColumn, Index index) {

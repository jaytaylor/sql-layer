...
public class EmbeddedOperatorCompiler extends ServerOperatorCompiler
{
    ...
    public ExecutableStatement compileExecutableStatement(JDBCConnection conn, DMLStatementNode sqlStmt, List<ParameterNode> sqlParams, boolean getParameterNames, ExecuteAutoGeneratedKeys autoGeneratedKeys, EmbeddedQueryContext context) {
        ...
        if (result.isUpdate())
            return compileModifyStatement((PhysicalUpdate)result,
                                          parameterMetaData,
                                          autoGeneratedKeys);
        else
            return compileQueryStatement((PhysicalSelect)result,
                                         parameterMetaData);
    }

    protected ExecutableQueryOperatorStatement compileQueryStatement(PhysicalSelect select,
                                                                     JDBCParameterMetaData parameterMetaData) {
        Operator resultOperator = (Operator)select.getPlannable();
    ...
    protected ExecutableModifyOperatorStatement compileModifyStatement(PhysicalUpdate update,
                                                                       JDBCParameterMetaData parameterMetaData,
                                                                       ExecuteAutoGeneratedKeys autoGeneratedKeys) {
        Operator resultOperator = (Operator)update.getPlannable();
        ...
        if (update.isReturning()) {
            ...
            for (PhysicalResultColumn column : update.getResultColumns()) {
                columns.add((ResultColumn)column);
            }
        }

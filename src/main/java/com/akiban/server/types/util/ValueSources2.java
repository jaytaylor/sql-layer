...
public class ValueSources2
{
    private static int hash(AkType left, AkType right)
    {
        return (left.ordinal() << Integer.bitCount(right.ordinal())) | right.ordinal();
    }
    
    private static final 
    private static Map<AkType, Map<AkType, Comparator<ValueSource>>> map;
    ...
    static
    {
        ...
        // map 2)
        Map<AkType, Comparator<ValueSource>> m2 = new EnumMap(AkType.class);
        //double
        ...
        for (AkType right : Iterables.concat(floating, exact, bool, year, time, date, 
                datetime, millis, month))
            m2.put(right, d_c2);
        
        // big object
        for (AkType right : Iterables.concat(bigDec, bigInt, text))
            m2.put(right, dec_c2);
...
//     *              => cast both to DOUBLE (to avoid losing precision) then do comparison
        ...
        Map<AkType, Comparator<ValueSource>> m9 = new EnumMap(AkType.class);
        
        //8) cast both to double
        m9.put(BOOL, d_c2);
        m9.put(INTERVAL_MONTH, d_c2);
        
        // 9) varchar and millis
        m9.put(INTERVAL_MILLIS, new Comparator<ValueSource>()
        {
            @Override
            public int compare(ValueSource left, ValueSource right)
            {

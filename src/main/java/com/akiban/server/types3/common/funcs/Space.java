...
public class Space extends TOverloadBase
{
    public static TOverload create (TClass stringType, TClass intType)
    {
        return new Space(stringType, intType);
    }

    private final TClass stringType;
    private final TClass intType;
    
    Space(TClass stringType, TClass intType)
    {
    ...
    {
        char ret[] = new char[inputs.get(0).getInt32()];
        Arrays.fill(ret, ' ');
    ...
    public TOverloadResult resultType()
    {
        return TOverloadResult.custom(new TCustomOverloadResult()
        {
            ...
            public TInstance resultInstance(List<TPreptimeValue> inputs, TPreptimeContext context)
            {
                ...
                //, which is not defined yet, so we'd return the stringType instead
                if (length == null || length.isNull())
                    return stringType.instance();
                else
                    return stringType.instance(length.getInt32());
            }
        });
    }
}
